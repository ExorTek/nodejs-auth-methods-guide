...
const register = async (req, res) => {
  ...
  if (!username || !email) {
    throw new CustomError('username and email are required', 400, true, 'VALIDATION_ERROR');
  }
...
const login = async (req, res) => {
  ...
  const tokens = await createTokenPair({ userId: user._id.toString(), username: user.username, req });
  sendTokenResponse(res, { statusCode: 200, ...tokens, data: { user: formatUser(user) } });
};
...
const getMe = async (req, res) => {
  const user = await User.findById(req.userId).select('-password -currentChallenge');
