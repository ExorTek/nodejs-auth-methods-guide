...
/**
 * Generate access token with jti (JWT ID) for revocation support
 */
 ...
 * Fastify: uses request (not req) for headers
 */
...
/**
 * Send token response — Fastify version
 * reply.header() + reply.code().send() instead of Express res.set() + res.status().json()
 */
...
/**
 * Handle OAuth callback — ticket-based token delivery (Fastify version)
 * reply.redirect() instead of Express res.redirect()
 */
const handleOAuthCallback = async (request, reply, { user, isNewUser, url = CLIENT_URL }) => {
...
export { createTokenPair, createAccessToken, sendTokenResponse, handleOAuthCallback, formatUser };
